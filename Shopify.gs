...
class APICall 
{
  ...
  async GetRequest (repo) {
    ...
    let res = await UrlFetchApp.fetch(`${this.root}${repo}`, params);
    let responseCode = res.getResponseCode();
    // console.warn(`Response Code : ${responseCode} ---> ${RESPONSECODES[responseCode]}`);
    if(responseCode != 200 || responseCode != 201) {
      console.error(`Couldn't make GET request`);
      return false;
    } else {
  ...
  async PostRequest (repo, payload) {
    let params = {
      "method" : "POST",
      "headers" : { "Authorization" : "Basic " + Utilities.base64EncodeWebSafe(`${this.api_key}:${this.api_pass}`) },
      "contentType" : "application/json",
    ...
    // console.warn(`Response Code : ${responseCode} ---> ${RESPONSECODES[responseCode]}`);
    if(responseCode != 200 || responseCode != 201) {
      console.error('Failed to POST to Shopify');
      return false;
    } else {
      let content = JSON.parse(html.getContentText());
  ...
  async CreateOrder () {
    ...
    console.info(`ORDER SUMMARY ----> ${order}`);
    return await this.PostRequest(repo, order);
  }
  ...
  async GetCustomerByEmail(email) {
    ...
    let res = await this.GetRequest(repo);
    let user = res['customers'][0];
    if(!user) {
      console.error('User Shopify Account Does Not Exist. Please make a User Account on Shopify.');
      console.error(`Trying as internal sale.....`)
      return this.GetCustomerByEmail("jacobsinstitutestore@gmail.com");
    } else {
  ...
  async GetProductByID(productID) {
    ...
    const repo = `products/${productID}.json?${status}${fields}`;
    let res = await this.GetRequest(repo);
    let parsed = JSON.parse(res)['product'];
    if(!parsed) {
  ...
  async GetLastOrder() {
    ...
    let res = await this.GetRequest(repo);
    let parsed = JSON.parse(res)['orders'][0];
    console.info(`ORDER PLACED ----> TIME : ${parsed.created_at}\\n ORDER NUMBER : ${parsed.name}\\n TO : ${parsed.email}\\n FOR : $${parsed.total_price}`);
    return parsed;  
  }
...
const _testAnotherAPI = () => {
  const a = new APICall({});
  console.info(a.GetLastOrder());
}
